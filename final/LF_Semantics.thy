(*  Title:      LF_Semantics.thy
    ...
    The reconstructed rules are sound. The original root-scope omission is
    documented by counterexamples, and the corrected Fitch turnstile is proved
    sound under the explicit requirement that the conclusion is at top level.
*)
...
section \<open>The root-scope hole, and how it was closed\<close>

text \<open>An earlier @{const fitchWF} left the proof below well formed: a bare
  subproof whose assumption is not a premise, yet whose only line
  @{const fitchConclusion} reads as the conclusion of the whole proof.  It made
  the Fitch turnstile unsound --- @{term "[] \<turnstile>\<^sub>F Pf"} held for an
  arbitrary @{term Pf}, and an interpretation making @{term Pf} false refuted the
  sequent.  @{const concludesAtTop} is the conjunct that closes it.\<close>

definition openAssumptionFitch :: fitch_proof where
  "openAssumptionFitch = [FSub (Subproof 1 Pf [])]"

lemma openAssumptionFitch_rejected: "\<not> fitchCorrect openAssumptionFitch"
  by eval

lemma openAssumptionFitch_diagnosis:
  "fitchWellFormed openAssumptionFitch
     = Some (STR ''the proof does not end at the outermost level'')"
  by eval

text \<open>The new conjunct is not redundant: the witness satisfies every other
  condition and fails only this one.\<close>

text \<open>The empty body invites an objection: perhaps that is the whole defect,
  and requiring a subproof to contain something would restore soundness.  It
  would not.  Below the body is not empty and ends in a line, so
  @{const lastIsLineItem} is satisfied substantively rather than vacuously, and
  the proof still concludes inside a box it never discharges.  What
  @{const concludesAtTop} rules out is not the empty subproof but the open one,
  and no condition on subproof *bodies* can express that: the fault is at the
  top level, in where the proof stops.\<close>

definition openAssumptionReit :: fitch_proof where
  "openAssumptionReit = [FSub (Subproof 1 Pf [FLine 2 Pf (FReit 1)])]"

lemma openAssumptionReit_satisfies_the_original_six:
  "sorted_wrt (<) (map flNum (flatten openAssumptionReit))"
  "list_all noAssumeLinesItem openAssumptionReit"
  "list_all (\<lambda>fl. flRule fl = FPremise \<longrightarrow> flScope fl = [])
     (flatten openAssumptionReit)"
  "premisesFirst openAssumptionReit"
  "list_all lastIsLineItem openAssumptionReit"
  "list_all (citationOK openAssumptionReit) (flatten openAssumptionReit)"
  by eval+

text \<open>Yet it reports no premises and a conclusion of @{term Pf}, so without the
  seventh conjunct it would assert @{term "[] \<turnstile>\<^sub>F Pf"} for an
  arbitrary @{term Pf}, exactly as the empty witness did.\<close>

lemma openAssumptionReit_sequent:
  "fitchPremises openAssumptionReit = []"
  "fitchConclusion openAssumptionReit = Some Pf"
  "\<not> concludesAtTop openAssumptionReit"
  "\<not> fitchCorrect openAssumptionReit"
  by eval+

lemma openAssumptionFitch_fails_only_root_scope:
  "sorted_wrt (<) (map flNum (flatten openAssumptionFitch))"
  "list_all noAssumeLinesItem openAssumptionFitch"
  "premisesFirst openAssumptionFitch"
  "list_all lastIsLineItem openAssumptionFitch"
  "list_all (citationOK openAssumptionFitch) (flatten openAssumptionFitch)"
  "\<not> concludesAtTop openAssumptionFitch"
  by eval+

section \<open>Semantic soundness of the Fitch turnstile\<close>

text \<open>With the conclusion pinned to the outermost level the Fitch turnstile is
  sound, and the proof needs no new model theory: @{thm [source]
  theorem_4_forward} carries the proof to Lemmon, @{thm [source] proposition_5}
  bounds the conclusion's dependencies by the assumptions in scope at it, and
  @{const concludesAtTop} makes that scope the premises alone.  What
  @{const openPremises} reports of the image is then a sublist of what
  @{const fitchPremises} reports of the source, and @{thm [source]
  lemmon_semantic_soundness} finishes.\<close>

lemma modelEntails_mono:
  assumes "modelEntails I \<Gamma> \<phi>" and "set \<Gamma> \<subseteq> set \<Delta>"
  shows "modelEntails I \<Delta> \<phi>"
  using assms by (auto simp: modelEntails_def)

lemma flatten_nonempty_sem: "F \<noteq> [] \<Longrightarrow> flatten F \<noteq> []"
  by (cases F) (auto simp: flatten_def)

lemma delta_nonempty: "F \<noteq> [] \<Longrightarrow> \<delta> F \<noteq> []"
  using flatten_nonempty_sem delta_nums by (metis Nil_is_map_conv)

lemma delta_conclusion:
  assumes ne: "F \<noteq> []"
  shows "conclusion (\<delta> F) = fitchConclusion F"
proof -
  from ne have f: "flatten F \<noteq> []" by (rule flatten_nonempty_sem)
  from ne have d: "\<delta> F \<noteq> []" by (rule delta_nonempty)
  have "formula (last (\<delta> F)) = last (map formula (\<delta> F))"
    using d by (simp add: last_map)
  also have "\<dots> = last (map flFm (flatten F))" by (simp add: delta_fms)
  also have "\<dots> = flFm (last (flatten F))" using f by (simp add: last_map)
  finally show ?thesis using d f by (simp add: conclusion_def fitchConclusion_def)
qed

text \<open>\<open>\<delta>\<close> keeps line numbers and formulas in place, so each of its lines has a
  flattened line with the same number and the same formula.\<close>

lemma delta_line_pair:
  assumes "l \<in> set (\<delta> F)"
  shows "\<exists>fl \<in> set (flatten F). flNum fl = lineNumber l \<and> flFm fl = formula l"
proof -
  from assms obtain i where i: "i < length (\<delta> F)" and li: "l = \<delta> F ! i"
    by (metis in_set_conv_nth)
  have len: "length (\<delta> F) = length (flatten F)"
    using delta_nums by (metis length_map)
  with i have i': "i < length (flatten F)" by simp
  have "lineNumber l = map lineNumber (\<delta> F) ! i" using i li by simp
  also have "\<dots> = map flNum (flatten F) ! i" by (simp add: delta_nums)
  also have "\<dots> = flNum (flatten F ! i)" using i' by simp
  finally have n: "flNum (flatten F ! i) = lineNumber l" by simp
  have "formula l = map formula (\<delta> F) ! i" using i li by simp
  also have "\<dots> = map flFm (flatten F) ! i" by (simp add: delta_fms)
  also have "\<dots> = flFm (flatten F ! i)" using i' by simp
  finally have m: "flFm (flatten F ! i) = formula l" by simp
  from i' have "flatten F ! i \<in> set (flatten F)" by simp
  with n m show ?thesis by blast
qed

lemma delta_openPremises_sub:
  assumes wf: "fitchWF F" and ne: "F \<noteq> []"
  shows "set (openPremises (\<delta> F)) \<subseteq> set (fitchPremises F)"
proof -
  from ne have f: "flatten F \<noteq> []" by (rule flatten_nonempty_sem)
  from ne have d: "\<delta> F \<noteq> []" by (rule delta_nonempty)
  let ?fl = "last (flatten F)"
  from f have mem: "?fl \<in> set (flatten F)" by simp
  have num: "lineNumber (last (\<delta> F)) = flNum ?fl"
  proof -
    have "lineNumber (last (\<delta> F)) = last (map lineNumber (\<delta> F))"
      using d by (simp add: last_map)
    also have "\<dots> = last (map flNum (flatten F))" by (simp add: delta_nums)
    also have "\<dots> = flNum ?fl" using f by (simp add: last_map)
    finally show ?thesis .
  qed
  have top: "flScope ?fl = []" by (rule fitchWF_conclusion_top [OF wf ne])
  have sc: "scopeOf F (flNum ?fl) = set (premiseLines F)"
    using scopePath_eq [OF wf mem] top by (simp add: scopeOf_def)
  from d have "last (\<delta> F) \<in> set (\<delta> F)" by simp
  with proposition_5 [OF wf]
  have "references (last (\<delta> F)) \<subseteq> scopeOf F (lineNumber (last (\<delta> F)))" by blast
  with num sc have refs: "references (last (\<delta> F)) \<subseteq> set (premiseLines F)" by simp
  have "set (openPremises (\<delta> F)) = set (depFms (\<delta> F) (references (last (\<delta> F))))"
    using d by (simp add: openPremises_def)
  also have "\<dots> \<subseteq> set (depFms (\<delta> F) (set (premiseLines F)))"
    by (rule depFms_mono [OF refs])
  also have "\<dots> \<subseteq> set (fitchPremises F)"
  proof
    fix g assume "g \<in> set (depFms (\<delta> F) (set (premiseLines F)))"
    then obtain l where l: "l \<in> set (\<delta> F)" "lineNumber l \<in> set (premiseLines F)"
      and lf: "formula l = g" by (auto simp: depFms_def)
    from delta_line_pair [OF l(1)] obtain fl
      where fl: "fl \<in> set (flatten F)" "flNum fl = lineNumber l" "flFm fl = formula l"
      by blast
    from l(2) obtain fl' where fl': "fl' \<in> set (flatten F)" "flRule fl' = FPremise"
        "flNum fl' = lineNumber l" by (auto simp: premiseLines_def)
    from fitchWF_distinct [OF wf] have "inj_on flNum (set (flatten F))"
      by (simp add: distinct_map)
    with fl(1,2) fl'(1,3) have "fl = fl'" by (auto simp: inj_on_def)
    with fl'(1,2) fl(3) lf show "g \<in> set (fitchPremises F)"
      by (force simp: fitchPremises_def)
  qed
  finally show ?thesis .
qed

theorem fitch_semantic_soundness:
  fixes I :: "'a interp"
  assumes seq: "\<Gamma> \<turnstile>\<^sub>F \<phi>"
  shows "modelEntails I \<Gamma> \<phi>"
proof -
  from seq obtain F where ok: "fitchCorrect F"
      and pre: "set (fitchPremises F) = set \<Gamma>"
      and concl: "fitchConclusion F = Some \<phi>"
    by (rule FseqE)
  from ok have wf: "fitchWF F" by (simp add: fitchCorrect_def)
  have ne: "F \<noteq> []"
  proof
    assume "F = []"
    with concl show False by (simp add: fitchConclusion_def flatten_def)
  qed
  from theorem_4_forward [OF ok] have lc: "lemmonCorrect (\<delta> F)" .
  from ne have d: "\<delta> F \<noteq> []" by (rule delta_nonempty)
  from delta_conclusion [OF ne] concl have dc: "conclusion (\<delta> F) = Some \<phi>" by simp
  have "openPremises (\<delta> F) \<turnstile>\<^sub>L \<phi>" by (rule LseqI [OF lc d refl dc])
  from lemmon_semantic_soundness [OF this]
  have "modelEntails I (openPremises (\<delta> F)) \<phi>" .
  moreover from delta_openPremises_sub [OF wf ne] pre
  have "set (openPremises (\<delta> F)) \<subseteq> set \<Gamma>" by simp
  ultimately show ?thesis by (rule modelEntails_mono)
qed

text \<open>Both turnstiles are therefore sound.  The witness above is the reason
  the Fitch one needed @{const concludesAtTop}: nothing else in
  @{const fitchWF} looks at where the proof ends.\<close>
