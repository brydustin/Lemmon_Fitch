...
subsection \<open>The image ends at the outermost level\<close>

text \<open>@{const concludesAtTop} is what makes the image's conclusion an assertion
  rather than a formula reached under an assumption.  For an image it holds for a
  reason internal to Lemmon: a box is opened by an assumption and closed by a
  strictly later line, so the source's last line opens no box, and the image's
  last item is a line rather than a subproof.\<close>

lemma boxesClosedIn_boxesOf:
  assumes corr: "lemmonCorrect P" and ne: "P \<noteq> []"
  shows "boxesClosedIn (boxesOf P) P"
  unfolding boxesClosedIn_def
proof (intro ballI impI)
  fix l ac assume l: "l \<in> set P" and ac: "ac \<in> set (boxesOf P)"
    and hd: "fst ac = lineNumber l"
  from boxesOf_source [OF ac] obtain l0
    where l0: "l0 \<in> set P" "ac \<in> set (dischargePairs (justification l0))" by blast
  from dischargePairs_earlier(2) [OF corr l0(1) l0(2)] have lt: "snd ac < lineNumber l0" .
  from corr have srt: "sorted_wrt (<) (map lineNumber P)"
    by (simp add: lemmonCorrect_def)
  from l0(1) have "lineNumber l0 \<in> set (map lineNumber P)" by simp
  from sorted_wrt_less_last [OF srt this] ne
  have "lineNumber l0 \<le> lineNumber (last P)" by (simp add: last_map)
  with lt show "snd ac < lineNumber (last P)" by simp
qed

theorem lemmonToFitchDirect_concludesAtTop:
  assumes A: "lemmonToFitchDirect P = Inr F"
  shows "concludesAtTop F"
proof (cases "P = []")
  case True
  with A have "F = []"
    by (auto simp: lemmonToFitchDirect_def split: option.splits if_splits)
  then show ?thesis by (simp add: concludesAtTop_def)
next
  case False
  from A have corr: "lemmonCorrect P" and F: "F = buildItems (boxesOf P) P"
      and bo: "boxOrderError P = None"
    by (auto simp: lemmonToFitchDirect_def split: option.splits if_splits)
  have ord: "\<forall>ac \<in> set (boxesOf P). fst ac \<le> snd ac"
    using boxOrderError_None [OF bo] by blast
  from buildItems_last [OF False ord boxesClosedIn_boxesOf [OF corr False]]
  obtain n f r where "last (buildItems (boxesOf P) P) = FLine n f r" by blast
  moreover from False have "buildItems (boxesOf P) P \<noteq> []"
    by (rule buildItems_nonempty)
  ultimately show ?thesis using F by (simp add: concludesAtTop_def)
qed
...
theorem lemmonToFitchDirect_fitchWF_iff_citations:
  ...
  using lemmonToFitchDirect_sorted [OF A] lemmonToFitchDirect_noAssume [OF A]
        ...
        lemmonToFitchDirect_lastIsLine [OF A]
        lemmonToFitchDirect_concludesAtTop [OF A]
  by blast
