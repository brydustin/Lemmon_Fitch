...
text \<open>Both proofs close with the same top-level line.  Without it the wide form
  would end inside the box opened at 2, and @{const concludesAtTop} would reject
  it: its conclusion would be a formula reached under the undischarged
  assumption @{term Qf} rather than one the proof asserts.  The point of the
  corollary is untouched --- the two proofs still differ only in how wide the
  subproof is, and \<open>\<delta>\<close> still identifies them.\<close>

definition cor6_wide :: fitch_proof where
  "cor6_wide =
     [ \<langle>1, Pf, FPremise\<rangle>\<^sub>F
     , \<lbrakk>2 : Qf; [\<langle>3, Pf \<or>\<^sub>o Rf, FOrIntroL 1\<rangle>\<^sub>F]\<rbrakk>\<^sub>F
     , \<langle>4, Pf \<or>\<^sub>o Rf, FOrIntroL 1\<rangle>\<^sub>F ]"
...
text \<open>This is the executable half only: that the direct construction rejects
  both examples.  It is deliberately *not* called \<open>theorem_10\<close>, because the
  paper's Theorem 10 is the stronger claim that no well-formed positional image
  exists at all, and that is proved as @{text theorem_10} in
  \<open>LF_Theorem10.thy\<close>.  Two theorems of the same name making different claims
  would leave a reader at the mercy of import order.\<close>

theorem examples_rejected_by_direct_construction:
