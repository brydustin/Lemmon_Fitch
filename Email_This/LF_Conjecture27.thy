...
section \<open>The late premise has no positional image at all\<close>

text \<open>This section used to record a counterexample to the converse of
  @{thm [source] lemmonToFitchDirect_fitchWF}.  The permissive assumption clause
  in @{const ruleMatches} lets an undischarged Lemmon assumption carry
  @{const FAssume}, and representing the late assumption of @{const premLate} as
  an unused one-line subproof produced a positional image that the direct
  checker rejects --- so the seven checks looked sufficient but not necessary.

  That image ended \emph{inside} the unused subproof, and
  @{const concludesAtTop} now rules it out.  The dodge is not merely blocked in
  this one shape: it cannot be repaired.  @{const premLate} ends in an
  undischarged assumption, so any positional image ends in a line matching it.
  That line stands at the outermost level, where an @{const FAssume} cannot
  stand, so it carries @{const FPremise} --- and it follows line 2, which is not
  a premise, so @{const premisesFirst} fails.  For this source the checker and
  the existence question agree after all.\<close>
...
lemma premLate_fitch_alt_now_rejected:
  "positionalImage premLate_fitch_alt premLate"
  "\<not> fitchWF premLate_fitch_alt"
  "\<not> concludesAtTop premLate_fitch_alt"
  by eval+

text \<open>An assumption line of the flattening always sits inside the box it opens,
  so its scope is not empty.\<close>

lemma assume_scope_nonempty:
  assumes wf: "fitchWF F" and m: "fl \<in> set (flatten F)" and r: "flRule fl = FAssume"
  shows "flScope fl \<noteq> []"
proof -
  from m obtain it where it: "it \<in> set F" and fm: "fl \<in> set (flatItem it [])"
    by (auto simp: flatten_def)
  from wf it have na: "noAssumeLinesItem it"
    by (auto simp: fitchWF_def list_all_iff)
  from r obtain n f sc where fl: "fl = FL n f FAssume sc" by (cases fl) simp
  from fm fl have "FL n f FAssume sc \<in> set (flatItem it [])" by simp
  from flat_assume_scope(1) [OF this na] have "n \<in> set sc" .
  with fl show ?thesis by auto
qed

theorem premLate_no_positional_image:
  "\<not> (\<exists>F. fitchWF F \<and> positionalImage F premLate)"
proof (rule notI, elim exE conjE)
  fix F assume wf: "fitchWF F" and img: "positionalImage F premLate"
  from img have "list_all2 lineMatches (flatten F) premLate"
    by (simp add: positionalImage_def)
  then obtain fl1 fl2 fl3 where flat: "flatten F = [fl1, fl2, fl3]"
      and m2: "lineMatches fl2 (ProofLine 2 (Disj Pf Rf) (OrIntroL 1) {1})"
      and m3: "lineMatches fl3 (ProofLine 3 Qf Assumption {3})"
    by (auto simp: premLate_def list_all2_Cons2 list_all2_Nil2)
  have ne: "F \<noteq> []"
  proof
    assume "F = []"
    with flat show False by (simp add: flatten_def)
  qed
  \<comment> \<open>the last line stands at the outermost level, so it is not an assumption\<close>
  from fitchWF_conclusion_top [OF wf ne] flat have sc: "flScope fl3 = []" by simp
  from flat have mem3: "fl3 \<in> set (flatten F)" by simp
  have "flRule fl3 \<noteq> FAssume"
    using assume_scope_nonempty [OF wf mem3] sc by auto
  moreover from m3 have "ruleMatches Assumption (flRule fl3)"
    by (simp add: lineMatches_def)
  ultimately have p3: "flRule fl3 = FPremise" by (simp add: ruleMatches_def)
  \<comment> \<open>but line 2 is not a premise, and it comes first\<close>
  from m2 have "ruleMatches (OrIntroL 1) (flRule fl2)" by (simp add: lineMatches_def)
  then have p2: "flRule fl2 \<noteq> FPremise" by (simp add: ruleMatches_def)
  from wf have pf: "premisesFirst F" by (simp add: fitchWF_def)
  show False
  proof (cases "flRule fl1 = FPremise")
    case True with pf flat p2 p3 show False by (simp add: premisesFirst_def)
  next
    case False with pf flat p2 p3 show False by (simp add: premisesFirst_def)
  qed
qed

theorem premLate_checks_agree:
  "lemmonCorrect premLate
     \<and> (\<nexists>F. fitchWF F \<and> positionalImage F premLate)
     \<and> (\<nexists>F. lemmonToFitchDirect premLate = Inr F)"
  using premLate_correct premLate_no_positional_image premLate_rejected by auto

text \<open>The converse of @{thm [source] lemmonToFitchDirect_fitchWF} is therefore
  open again.  It was refuted for the old @{const fitchWF}; with
  @{const concludesAtTop} in force, no counterexample is known.  Every witness
  the seven checks were built from is now known to have no image at all, which
  is evidence for the converse but not a proof of it: those are six sources, and
  the converse quantifies over all of them.\<close>
