...
module
  LemmonFitch(Num, Int, Nat, Trm(..), Fm(..), Justa(..), Hl_term(..),
               Hl_justification(..), Hl_formula(..), Set, Char, Sum,
               Fitch_rule(..), Fline(..), Pline(..), Route(..), Subproof(..),
               Fitch_item(..), Deriv(..), Drule(..), Hl_line(..),
               Translation_error(..), Hl_route(..), Hl_fitch_rule(..),
               Hl_subproof(..), Hl_fitch_item(..), Hl_derivation(..),
               Hl_derivation_rule(..), Hl_finite_model(..),
               Hl_translation_error(..), rn_t, rn, names_t, names, openAsms,
               inst_t, inst, fvs_t, fvs, instWitnesses, rnD, lookupLine, fmAt,
               eqsub_t, eqsub, genOK, witOK, flatten, premisesFirst,
               fCitedLines, fCitedSubs, subrefs, citationOK, fitchWF,
               premiseLines, scopePath, scopeOf, depFms, depsAt, citedLines,
               depsOf, instOK, justAt, ruleOK, showFm, toLemmonRule, delta,
               dischargePairs, boxesOf, boxPath, showNat, derivationToFitch,
               lineOK_gen, checkFrom_gen, lemmonCorrect, toDerivation, viaTree,
               recompute, stripDeps, showJust, sentence, lemmon_21, showError,
               showFitch, toFitchRule, buildItems, scopeError, fresh_for,
               conclusion, showLemmon, fitchCorrect, derivOK, hlCitedLines,
               hlStructureOK, hlQuantifierNegationForms,
               ...
               hlContradiction, hlFormulaAt, hlRuleOK, hlLineOK, fitchToLemmon,
               boxHeadError, nestingError, premiseError, fitchPremises,
               hlCorrect, openPremises, renumberJust, relabel, boxOrderError,
               subScopeError, premiseOrderError, lemmonToFitchDirect,
               lemmonToFitch, fitchConclusion, fitchWellFormed, fresh_for_fms,
               hlIsPropositional, hlLiteralFormula, hlDisjoin, hlConjoin,
               hlClausesFormula, hlDNFClauses, hlEliminateImplications,
               hlPushNegations, hlToNNF, hlToDNF, hlConclusion, showTranslation,
               dependents, ruleMatches, lineMatches, fitchImage, hlOpenPremises,
               permuteProof, hlSubBody, noDuplication, sourceCovered,
               ...
               hlDependencyClosed, hlVerifiedCorrect, hlToDerivation, hlViaTree,
               hlJustificationAt, hlFreeFor, lemmonToFitchChecked,
               positionalImage, hlIsAssumptionLine, hlFitchLineNumbers,
               hlSubLastLine, hlBoxPath, hlFlattenFitch, hlItemLastLine,
               hlToLemmonRule, hlIsPrefix, hlFitchCitedLines,
               hlFitchDependenciesOf, hlDependencyLookup, hlFitchToLemmonFrom,
               hlFitchToLemmon, hlFitchScopeFrom, hlFitchWellFormed,
               hlFitchVerified, hlOptionAll, hlOptionAny, hlFinitePredicates,
               hlFiniteDomain, hlFiniteConstants, hlFiniteEvalTerm,
               hlFiniteEval, hlFirstDifference, hlToFitchRule, hlBadCitations,
               hlScopeError, hlSubproofLevel, hlBadSubproofCitations,
               hlSubproofScopeError, hlIsPremiseLine, hlPremiseOrderError,
               ...
               hlPrefixForalls, hlEmitDerivations, hlSubAssumptionLine,
               hlSubAssumptionFormula, hlVariablesInFormula, hlFiniteEvalClosed,
               hlDerivationConstants, hlLemmonToFitchChecked,
               hlFiniteStandardEquality)
  where {
...
data Fm = Atom String [Trm] | Eqf Trm Trm | Bot | Nega Fm | Conj Fm Fm
...
equal_fm (Nega x4) (Exi x101 x102) = False;
equal_fm (Exi x101 x102) (Nega x4) = False;
equal_fm (Nega x4) (Uni x91 x92) = False;
equal_fm (Uni x91 x92) (Nega x4) = False;
equal_fm (Nega x4) (Iff x81 x82) = False;
equal_fm (Iff x81 x82) (Nega x4) = False;
equal_fm (Nega x4) (Impl x71 x72) = False;
equal_fm (Impl x71 x72) (Nega x4) = False;
equal_fm (Nega x4) (Disj x61 x62) = False;
equal_fm (Disj x61 x62) (Nega x4) = False;
equal_fm (Nega x4) (Conj x51 x52) = False;
equal_fm (Conj x51 x52) (Nega x4) = False;
...
equal_fm Bot (Nega x4) = False;
equal_fm (Nega x4) Bot = False;
...
equal_fm (Eqf x21 x22) (Nega x4) = False;
equal_fm (Nega x4) (Eqf x21 x22) = False;
...
equal_fm (Atom x11 x12) (Nega x4) = False;
equal_fm (Nega x4) (Atom x11 x12) = False;
...
equal_fm (Nega x4) (Nega y4) = equal_fm x4 y4;
...
inst x a Bot = Bot;
inst x a (Nega p) = Nega (inst x a p);
...
eqsub a b (Nega pa) (Nega p) = eqsub a b pa p;
...
eqsub a b (Eqf v va) (Nega vb) = False;
...
eqsub a b (Nega v) (Atom va vb) = False;
eqsub a b (Nega v) (Eqf va vb) = False;
eqsub a b (Nega v) Bot = False;
eqsub a b (Nega v) (Conj va vb) = False;
eqsub a b (Nega v) (Disj va vb) = False;
eqsub a b (Nega v) (Impl va vb) = False;
eqsub a b (Nega v) (Iff va vb) = False;
eqsub a b (Nega v) (Uni va vb) = False;
eqsub a b (Nega v) (Exi va vb) = False;
eqsub a b (Conj v va) (Atom vb vc) = False;
eqsub a b (Conj v va) (Eqf vb vc) = False;
eqsub a b (Conj v va) Bot = False;
eqsub a b (Conj v va) (Nega vb) = False;
...
eqsub a b (Disj v va) (Nega vb) = False;
...
eqsub a b (Impl v va) (Nega vb) = False;
...
eqsub a b (Iff v va) (Nega vb) = False;
...
eqsub a b (Uni v va) (Nega vb) = False;
...
eqsub a b (Exi v va) (Nega vb) = False;
...
eqsub a b (Atom va vb) (Nega v) = False;
  ...
  sorted_wrt less_nat (map flNum (flatten f)) &&
    all noAssumeLinesItem f &&
      all (\ fl ->
        ...
        (flatten f) &&
        premisesFirst f &&
          all lastIsLineItem f && all (citationOK f) (flatten f);
...
ruleOK e phi asa (RAA a c) =
  isAssumptionLine e a &&
    fmAt e c == Just Bot && (case fmAt e a of {
                              Nothing -> False;
                              Just p -> equal_fm phi (Nega p);
                            });
ruleOK e phi asa (DN i) = fmAt e i == Just (Nega (Nega phi));
ruleOK e phi asa (BotI i j) =
  equal_fm phi Bot && (case fmAt e i of {
                        Nothing -> False;
                        Just p -> fmAt e j == Just (Nega p);
                      });
...
showFm Bot = symBot;
showFm (Nega Bot) = isaSym "top";
showFm (Nega (Atom v va)) = symNot ++ showFm (Atom v va);
showFm (Nega (Eqf v va)) = symNot ++ showFm (Eqf v va);
showFm (Nega (Nega v)) = symNot ++ showFm (Nega v);
showFm (Nega (Conj v va)) = symNot ++ showFm (Conj v va);
showFm (Nega (Disj v va)) = symNot ++ showFm (Disj v va);
showFm (Nega (Impl v va)) = symNot ++ showFm (Impl v va);
showFm (Nega (Iff v va)) = symNot ++ showFm (Iff v va);
showFm (Nega (Uni v va)) = symNot ++ showFm (Uni v va);
showFm (Nega (Exi v va)) = symNot ++ showFm (Exi v va);
showFm (Conj p q) =
...
derivOK (Deriv phi (DRAA a fa d)) =
  derivOK d &&
    equal_fm phi (Nega fa) && equal_fm (dForm d) Bot && dischargeOK a fa d;
derivOK (Deriv phi (DDN d)) = derivOK d && equal_fm (dForm d) (Nega (Nega phi));
derivOK (Deriv phi (DBotI d1 d2)) =
  derivOK d1 &&
    derivOK d2 && equal_fm phi Bot && equal_fm (dForm d2) (Nega (dForm d1));
           ...
           then Just "an assumption occurs otherwise than as the head of a subproof"
                  ...
                  then Just "a premise occurs inside a subproof"
                         ...
                         then Just "a premise occurs after a line that is not a premise"
                                ...
                                else (if not (all (citationOK f) (flatten f))
                                       then Just
      "a line cites something not in its scope"
                                       else Nothing))))));
...
int_of_integer k =
  (if k == (0 :: Integer) then Zero_int
    else (if k == (-1 :: Integer) then uminus_int one_int
...
hlDerivationToFitch d =
  ...
  } in (case hlEmitDerivation base env []
...
hlViaTree p = (case hlToDerivation p of {
                Nothing -> Inl (HL_SourceNotVerified Zero_int);
                Just d -> Inr (HL_ViaTreeRoute, hlDerivationToFitch d);
              });
...
hlFitchVerified f =
  hlFitchWellFormed f && hlVerifiedCorrect (hlFitchToLemmon f);
...
hlGeneralizedConstants p (HL_ProofLine v va (HL_ExistsElim vd ve vf) vc) = [];
