...
- Complete 40-theory cap and Haskell/SML regeneration: **passed**, source-loaded
  from HOL by `./generate_code.sh` (5m08s overall, 5m06s session time, exit 0). All three
...
The maintained sources at that 40-theory checkpoint contain 917 lemma/theorem/corollary command sites and
...
status 0. The subsequent 40-theory source check regenerated all three exports
byte-for-byte unchanged, so these executable results continue to apply:
