...
text \<open>
  ...
  is fixed and is reproduced exactly:
    ...
    \<^item> exactly one rule, @{text ForallIntro}, imposes a side condition on the
      assumptions the cited line rests on rather than on the formulas.

  Everything downstream depends only on those three facts, so changing the
  roster means editing @{text citedLines}, @{text dischargePairs},
  @{text depsOf} and @{text ruleOK} in this file, and their Fitch
  counterparts, and nothing else.
