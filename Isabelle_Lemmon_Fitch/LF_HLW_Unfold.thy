...
theory LF_HLW_Unfold
  imports LF_HLW_Translate
begin
     ...
     (let env = hlPremiseEnvironment d;
          ...
          (body,conclusion,next,count) =
            hlEmitDerivation base env (map (\<lambda>(source,n,phi). phi) env)
              (1 + int (length env)) 0 d
      in premiseLines @ body)"

section \<open>The checked exact translation\<close>
