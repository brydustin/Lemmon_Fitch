...
object into a correct Lemmon proof. A separate semantic audit shows that the
present Fitch turnstile also admits a proof ending in an undischarged
top-level subproof. These machine-found boundary cases are stated explicitly
rather than hidden by a round-trip test. Translation in the other direction is not
...
We give a construction translating any correct Lemmon proof by way of its
unfolding. Fresh-name repair handles the eigenvariable conditions for both
universal introduction and existential elimination. In Isabelle/HOL we prove
that the repaired construction terminates and yields a correct Fitch proof
with the same conclusion. We also answer two economy questions negatively. A
...
in a well-formed Fitch proof. A further formal counterexample shows that
laminar dependency regions do not suffice. The Isabelle definitions generate
...
\verb|Halvorson/lemmon-checker-main/src| is authoritative for the rule language
...
handwritten program was partial.
\end{abstract}
...
translation proofs. Section~\ref{sec:machine} explains the correspondence and
the defects that total formal functions made visible.
...
For Fitch proofs satisfying the intended closed-root convention, the round
trip through Lemmon and the checked reverse translation behaves as a
normalisation rather than an identity: it may return a proof whose subproofs
are no wider than they need to be.
...
above, using a globally fresh name whenever an eigenvariable would occur in an
assumption in scope. So modified, it yields for every nonempty correct Lemmon
proof a well-formed and correct Fitch proof with the same conclusion.
\end{theorem}
...
The four obligations suggested by the construction are now machine-checked.
In the Isabelle sources they appear as
...
Theorem~\ref{conj:total}, formerly the totality conjecture.
...
There is also a negative answer to the proposed laminarity criterion. The
dependency regions of Example~\ref{ex:trap} are disjoint, hence laminar, but
the Isabelle theorem \verb|theorem_10_ex12| proves that it has no Fitch image
at all. Laminar dependency sets are therefore not sufficient for a
...
The repaired totality claim is a theorem over arbitrary source proofs, not an
extrapolation from examples:
...
with the earlier draft; its Isabelle status is \verb|theorem|. The other
...
\verb|hlLemmonToFitchChecked| first accepts a direct image only when the exact
Fitch checker verifies it, and otherwise unfolds the citation graph, repairs
eigenconstants, and lays out nested subproofs.  An executable shared-discharge
example forces this fallback and verifies both the resulting Fitch proof and
its conclusion after applying $\delta_H$.
...
Before the universal results were proved, the translations were implemented
in Haskell against the proof checker for \emph{How Logic Works}. Those tests
remain useful as concrete validation of the parser, checker, renderer, and
examples, but they are no longer being asked to stand in for a proof. The
...
Fitch proof. Isabelle records it as \verb|fitch_semantic_soundness_fails|.
The result should be read
constructively: the Fitch checker needs a closed-root condition if its
turnstile is to take the premises of the outer level as the assumptions of the
whole proof. Formalisation is valuable here precisely because it does not
silently supply the intended convention.

\subsection{Generated Haskell and SML}

The cap theory \verb|LF_All.thy| is also the code-generation entry point.
...
Translation from correct Fitch proofs to Lemmon proofs is total, canonical,
...
nor a duplication-free target can serve in general.

It would be premature to describe the current formal objects as a retract.
That formulation requires the converse direction of the original
Theorem~\ref{thm:total}, and Remark~\ref{rem:theorem4-converse} gives a
counterexample to it. Once the intended closed-root condition is made part of
the Fitch turnstile, the appropriate normalisation or retract theorem can be
stated and proved against that strengthened notion. What is already proved is
the substantive translation result: every correct Lemmon proof has a correct
Fitch derivation with the same conclusion, and every correct Fitch proof has a
correct Lemmon image.
